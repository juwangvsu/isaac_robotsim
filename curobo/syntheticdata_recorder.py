...
class Extension(omni.ext.IExt):
    ...
    def _build_window_ui(self):
        ...
        sensor_settings_ui_default = {
            ...
            # TODO
            "bbox_3d":{"checkbox": None, "colorize": None, "npy": None},
        }
                ...
                for viewport_name in self._viewport_names:
                    ...
                    with ui.CollapsableFrame(viewport_name + ": Sensor Settings", height=0):
                        with ui.VStack(spacing=5):
                            ...
                            # TODO
                            def toggle_camera(self, viewport_name, value):
                                self._sensor_settings[viewport_name]["camera"]["enabled"] = value
                            ...
                            def toggle_rgb_sensor(self, viewport_name, value):
                                ...
                                if value == False:
                                    ...
                                    # TODO
                                    self._sensor_settings_ui[viewport_name]["bbox_3d"]["colorize"].enabled = value
                                else:
                                    ...
                                    if self._sensor_settings[viewport_name]["bbox_3d"]["enabled"]:
                                        self._sensor_settings_ui[viewport_name]["bbox_3d"][
                                            "colorize"
                                        ].enabled = value
                            ...
                            # TODO
                            def toggle_bbox_3d_sensor(self, viewport_name, value):
                                self._sensor_settings[viewport_name]["bbox_3d"]["enabled"] = value
                                self._sensor_settings_ui[viewport_name]["bbox_3d"]["colorize"].enabled = value
                                self._sensor_settings_ui[viewport_name]["bbox_3d"]["npy"].enabled = value
                                if value:
                                    self._sensor_settings_ui[viewport_name]["bbox_3d"][
                                        "colorize"
                                    ].enabled = self._sensor_settings[viewport_name]["rgb"]["enabled"]
                                    asyncio.ensure_future(
                                        self.sd_helper.initialize_async(
                                            viewport, [self.sd_helper.sd.SensorType.BoundingBox3D] # 查 help(sd_helper.sd.SensorType) 有BoundingBox3D
                                        )
                                    ) 
                            # TODO
                            def toggle_bbox_3d_colorize(self, viewport_name, value):
                                self._sensor_settings[viewport_name]["bbox_3d"]["colorize"] = value
                            # TODO
                            def toggle_bbox_3d_npy(self, viewport_name, value):
                                self._sensor_settings[viewport_name]["bbox_3d"]["npy"] = value
                            ...
                            with ui.HStack(height=30):
                                ui.Spacer(width=10)
                                ui.Label("Camera", height=0, width=150)
                                self._sensor_settings_ui[viewport_name]["camera"]["checkbox"] = ui.CheckBox()
                                self._sensor_settings_ui[viewport_name]["camera"]["checkbox"].model.add_value_changed_fn(
                                    lambda a, v=viewport_name, this=self: toggle_camera(
                                        self, v, a.get_value_as_bool()
                                    )
                                )
                            ...
                            with ui.HStack(height=30):
                                ui.Spacer(width=10)
                                bbox_3d_label = ui.Label("3D Bounding Box", height=0, width=150)
                                bbox_3d_label.set_tooltip("To colorize sensor output, enable RGB")
                                self._sensor_settings_ui[viewport_name]["bbox_3d"]["checkbox"] = ui.CheckBox(
                                    width=75
                                )
                                self._sensor_settings_ui[viewport_name]["bbox_3d"][
                                    "checkbox"
                                ].model.add_value_changed_fn(
                                    lambda a, v=viewport_name, this=self: toggle_bbox_3d_sensor(
                                        self, v, a.get_value_as_bool()
                                    )
                                )
                                self._sensor_settings_ui[viewport_name]["bbox_3d"]["colorize"] = ui.CheckBox(
                                    enabled=False, width=75
                                )
                                self._sensor_settings_ui[viewport_name]["bbox_3d"][
                                    "colorize"
                                ].model.add_value_changed_fn(
                                    lambda a, v=viewport_name, this=self: toggle_bbox_3d_colorize(
                                        self, v, a.get_value_as_bool()
                                    )
                                )
                                self._sensor_settings_ui[viewport_name]["bbox_3d"]["npy"] = ui.CheckBox(
                                    enabled=False, width=75
                                )
                                self._sensor_settings_ui[viewport_name]["bbox_3d"][
                                    "npy"
                                ].model.add_value_changed_fn(
                                    lambda a, v=viewport_name, this=self: toggle_bbox_3d_npy(
                                        self, v, a.get_value_as_bool()
                                    )
                                )
    ...
    def _update(self, e: carb.events.IEvent):
        ...
        dt = e.payload["dt"]            # Payload 是一个类似字典的具有任意数据的项目
        ...
        # define data_writer = DataWriter()
        # TODO--> DataWriter--> writer.py
        ...
        for viewport_name in self._viewport_names:
            ...
            groundtruth = {
                "METADATA": {
                    ...
                    # TODO
                    "BBOX3D": {},
                    # TODO save Camera_params?
                    #"CAMERA": {},
                },
                "DATA": {},
            }
            # gt_list: sensors which are activated
            gt_list = []
            ...
            # 其中 gt["camera"] = self.sensor_helpers["camera"](viewport) >>f: self.get_camera_params
            # 得到这个相机的数据字典
            # Camera
            if self._sensor_settings[viewport_name]["camera"]["enabled"]:
            ...
            if self._sensor_settings[viewport_name]["bbox_3d"]["enabled"] and gt["state"]["boundingBox3D"]:
                ...
                groundtruth["METADATA"]["BBOX3D"]["COLORIZE"] = self._sensor_settings[viewport_name][
                    "bbox_3d"
                ]["colorize"]
                groundtruth["METADATA"]["BBOX3D"]["NPY"] = self._sensor_settings[viewport_name]["bbox_3d"][
                    "npy"
                ]
