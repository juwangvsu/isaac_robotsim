...
omni.kit.pipapi.install(
    ...
    #version="2.13.0",
    module="cspace", # sometimes module is different from package name, module is used for import check
...
panda_pose = loadtxt("/home/student/Documents/tang/VisionRobot/isaac_example/panda_pose.csv", comments="#", delimiter=",", unpack=False)

import sys
print('python version ', sys.version)
import cspace.cspace.classes
'''
kinematics = cspace.cspace.classes.Kinematics('/tmp/panda.urdf', "panda_hand", base=None)
...
print("POSE: ", list((link, pose.position(link)) for link in kinematics.link))
'''
...
class HelloWorld(BaseSample):
    ...
    async def setup_post_load(self):
        ...
        diffuse_tx.CreateInput('file', Sdf.ValueTypeNames.Asset).Set("/home/student/isaac-ground-texture.png")
    ...
    #random x, y position, 1.5, 0, -1.5, z  at 1.5 1.8
    def camera_xyz(self):
        xs =[-1.5, 0, 1.5]
        ys =[-1.5, 0, 1.5]
        zs =[1.5, 1.8]
        return [random.choice(xs), random.choice(ys), random.choice(zs)]
    ...
    def nextsession(self):
        ...
        camera_position=self.camera_xyz()
        from pxr import Gf
        import omni.usd
        ...
        new_location_np = np.array([target_t[0], target_t[1], target_t[2]])
        #random generate new position
        new_location_np = np.array([x_sign*(target_t[0]+x_r), y_sign*(target_t[1]+y_r), target_t[2]])

        #or use a entry from file gen by cspace
        new_location_np[0]=panda_pose[1024-self.runnum][0]
        new_location_np[1]=panda_pose[1024-self.runnum][1]
